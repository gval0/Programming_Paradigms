...
/* display generated text more elegantly */
void displayResult(vector<string>& res){
  int curCharsPerLine = 0;
  if(res.size() < 1) return;
  cout << "    " << res[0];
  for(int i = 1; i < res.size(); i++){
    cout << " ";
    if(curCharsPerLine >= 40) {
      cout << endl;
      curCharsPerLine = 0;
    }
    cout << res[i];
    curCharsPerLine += res[i].size();
  }
  cout << endl;
}

/* recursive function, makes expansions */
void expand(string str, map<string, Definition>& grammar, vector<string>& res){
  if(grammar.find(str) == grammar.end()){
      res.push_back(str);
      return;
  }
  Production curProd = grammar[str].getRandomProduction();
  for(Production::iterator cur = curProd.begin(); cur != curProd.end(); cur++){
    expand(*cur, grammar, res);
  }
}

/* generates random sentances based on grammar */
void dorsg(map<string, Definition>& grammar){
  for(int i = 0; i < 3; i++){
    cout << "Version #";
    cout << (i+1);
    cout << ": ---------------------------" << endl; //print version number
    vector<string> res;
    expand("<start>", grammar, res);
    displayResult(res);
  }
}
/**
