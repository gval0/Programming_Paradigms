          ...
          int numWorkers)
{
  ...
  bank->workingNum = numWorkers;
  sem_init(&(bank->bankLock), 0, 1);
  sem_init(&(bank->doingReport), 0, 0);
  sem_init(&(bank->repTrans), 0, 1);
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for(int i = 0; i < bank->numberBranches; i++) pthread_mutex_lock(&(bank->branches[i].branchLock));
  ...
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
    ...
    if (err < 0) {
      for(int i = 0; i < bank->numberBranches; i++) pthread_mutex_unlock(&(bank->branches[i].branchLock));
      return err;
    }
