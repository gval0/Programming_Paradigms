...
(define (quicksort num-list)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (caar num-list) (cdr num-list))))
...
;; MY CODE FOR THE ASSIGMENT -------------

;; intersection-points
(define (intersection-points l)
  (apply append
    (map (lambda (cur_circles) (intersect (car cur_circles) (cadr cur_circles))) (k-subsets l 2))
  )
)

;;distance-product
(define (distance-product point l)
  (apply * 
      (map (lambda (cur-point) (dist point cur-point)) (remove point l))
  )
)

;;rate-points
(define (rate-points l)
    (map (lambda (cur-point) (list (distance-product cur-point l) cur-point)) l)
)
...
;; type (()()) - for one element
;; returns list with just () type - somewhat
(define (useful-list l)
  (apply append (map (lambda (elem) (cdr elem)) (sort-points (rate-points l))))
)
;;clumped-points
(define (clumped-points l)
(if (= (remainder (length l) 2) 0)
    (prefix-of-list (useful-list l) (/ (length l) 2))
    (prefix-of-list (useful-list l) (/ (- (length l) 1) 2))
)
)

;; returns only x-value list
(define (get-x l)
  (apply + (map (lambda (cur) (x cur)) l))
)
;; returns only y-value list
(define (get-y l)
   (apply + (map (lambda (cur) (y cur)) l))
)
;;average-point
(define (average-point l)
  (let ((ultimate-x (/ (get-x l) (length l)))
        (ultimate-y (/ (get-y l) (length l)))
       )
        (list (distance-product (make-pt ultimate-x ultimate-y) l) (make-pt ultimate-x ultimate-y))
  )
)

;;best-estimate
(define (best-estimate l)
  (average-point (clumped-points (intersection-points l)))
)

;;where-am-i
(define (where-am-i l-dist l-stars)
  (sort-points (map best-estimate (all-guesses l-dist l-stars)))
)
