...
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *hashedWords, hashset *visitedText);
static void ProcessFeed(const char *remoteDocumentName, hashset *stopWords, hashset *hashedWords, hashset *visitedText);
static void PullAllNewsItems(FILE *dataStream, hashset *stopWords, hashset *hashedWords, hashset *visitedText);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *hashedWords, hashset *visitedText);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL, hashset *stopWords, hashset *hashedWords, hashset *visitedText);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL, hashset *stopWords, hashset *visitedText, hashset *hashedWords);
static void QueryIndices(hashset *stopWords, hashset *hashedWords, hashset *visitedText);
static void ProcessResponse(const char *word, hashset *stopWords, hashset *hashedWords, hashset *visitedText);
static bool WordIsWellFormed(const char *word);

// ------------------------------------------------------------------------
/* MY FUNCTIONS AND STUCTS */

// LOAD STOPWORDS FUNCTION
static void loadStopWords(hashset *hash);
...
static int strCompare(void *s1, void *s2)
{
  char *elem1 = *(char **)s1;
  tolower(elem1);
  char *elem2 = *(char **)s2;
  tolower(elem2);
  int res = strcmp(elem1, elem2);
  return res;
}

// string freeing function
static void strFree(void *elem)
{
  free(*(char **)elem);
}
/* STRING SHIT ENDS*/
...
static int textCompare(void *elem1, void *elem2)
{
  char *url1 = ((text *)elem1)->URL;
  char *url2 = ((text *)elem2)->URL;
  char *title1 = ((text *)elem1)->title;
  char *title2 = ((text *)elem2)->title;
  tolower(url1);
  tolower(url2);
  tolower(title1);
  tolower(title2);
  if (strcmp(url1, url2) == 0 || strcmp(title1, title2) == 0)
    return 0;
  else
    return 1;
}

static int textNumbersCompare(void *elem1, void *elem2)
{
  int *res1 = ((text *)elem1)->wordsN;
  int *res2 = ((text *)elem2)->wordsN;
  int res = res2 - res1;
...
static void sortFunction(void *elem, void *aux)
{
  wordHash *cur = (wordHash *)elem;
  VectorSort(cur->texts, textNumbersCompare);
}

// ----------------------------------------

static int wordsHashHash(void *elem, int bucketsN)
{
  int i;
  unsigned long hashcode = 0;
  char *str = ((wordHash *)elem)->keyWord;
  for (i = 0; i < strlen(str); i++)
    hashcode = hashcode * kHashMultiplier + tolower(str[i]);
  return hashcode % bucketsN;
}

static int wordHashCompare(void *elem1, void *elem2)
{
  char *e1 = ((wordHash *)elem1)->keyWord;
  tolower(e1);
  char *e2 = ((wordHash *)elem2)->keyWord;
  tolower(e2);
  int res = strcmp(e1, e2);
  return res;
}

static void wordHashFree(void *elem)
{
  char *word = ((wordHash *)elem)->keyWord;
  free(word);
  vector *curvec = ((wordHash *)elem)->texts;
  VectorDispose(curvec);
  free(curvec);
}
//-------------------------------------------------------------------------
 ...
 * word appears.
 */
static const int bucketsN = 1009;
...
static const char *const kTextDelimiters =
    " \t\n\r\b!@$%^*()_+={[}]|\\'\":;/?.>,<~`";
static const char *const stopWordsF = "data/stop-words.txt";
static const char *const kNewLineDelimiters = "\r\n";

// EXTRA FUNCTIONS ---------------------
static void getStopWords(hashset *hash)
{
  FILE *file = fopen(stopWordsF, "r");
  assert(file != NULL);

  streamtokenizer stokenizer;
  char buffer[1024];

  STNew(&stokenizer, file, kNewLineDelimiters, true);
  while (STNextToken(&stokenizer, buffer, sizeof(buffer)))
  {
    char *str = strdup(buffer);
    HashSetEnter(hash, &str);
  }
  STDispose(&stokenizer);
  fclose(file);
}
...
int main(int argc, char **argv)
{
  ...
  hashset hashedWords;
  hashset visitedText;
  HashSetNew(&stopWords, sizeof(char *), bucketsN, StringHash, strCompare, strFree);
  HashSetNew(&hashedWords, sizeof(wordHash), bucketsN, wordsHashHash, wordHashCompare, wordHashFree);
  HashSetNew(&visitedText, sizeof(char *), bucketsN, StringHash, strCompare, strFree);
  ...
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &stopWords, &hashedWords, &visitedText);
  QueryIndices(&stopWords, &hashedWords, &visitedText);
  curl_global_cleanup();

  HashSetDispose(&stopWords);
  HashSetDispose(&hashedWords);
  HashSetDispose(&visitedText);
  return 0;
}
...
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  ...
  { // ignore everything up to the first selicolon of the line
    ...
    ProcessFeed(remoteFileName, stopWords, hashedWords, visitedText);
  }
  HashSetMap(hashedWords, sortFunction, NULL);
  STDispose(&st);
  fclose(infile);
  printf("\n");
...
static void ProcessFeedFromFile(char *fileName, hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName, stopWords, visitedText, hashedWords);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName, hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  ...
  {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), stopWords, hashedWords, visitedText);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, stopWords, hashedWords, visitedText);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream, hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  ...
  { // if true is returned, then assume that <item ...> has just been
    ...
    ProcessSingleNewsItem(&st, stopWords, hashedWords, visitedText);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, stopWords, hashedWords, visitedText);
}
                         ...
                         const char *articleURL, hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL, stopWords, visitedText, hashedWords);
  STDispose(&st);
  fclose(tmpDoc);
}
 ...
 */
static bool compareTexts(hashset *visitedText, char *articleTitle, char *articleURL)
{
  char *url = strdup(articleURL);
  char *title = strdup(articleTitle);
  bool res = false;
  if ((HashSetLookup(visitedText, &url) != NULL) || (HashSetLookup(visitedText, &title) != NULL))
    res = false;
  if ((HashSetLookup(visitedText, &url) == NULL) && (HashSetLookup(visitedText, &title) == NULL))
    res = true;
  free(title);
...
static void makeText(text *Text, char *url, char *title, int num)
{
  (*Text).title = title;
  (*Text).URL = url;
  (*Text).wordsN = num;
...
static void makeWordsHash(wordHash *words, char *key)
{
  tolower(key);
  words->keyWord = key;
}

static void add(hashset *hashedWords, char *curWord, char *articleTitle, char *articleURL, hashset *stopWords, hashset *visitedText)
{
  tolower(curWord);
  vector *newVec = malloc(sizeof(vector));
  VectorNew(newVec, sizeof(text), textFree, bucketsN);
  text Text;
  char *url = strdup(articleURL);
  char *title = strdup(articleTitle);

  // possible change 
   makeText(&Text, url, title, 1);
  VectorAppend(newVec, &Text);

  wordHash words;
  char *key = strdup(curWord);
  // possible change 
  makeWordsHash(&words, key);

  words.texts = newVec;
  HashSetEnter(hashedWords, &words);
}

static void renew(char *articleTitle, char *articleURL, wordHash *wordsM, hashset *visitedText, hashset *stopWords)
{
  text Text;
  char *url = strdup(articleURL);
  char *title = strdup(articleTitle);
  ...
  int index = VectorSearch(wordsM->texts, &Text, textCompare, 0, false);

  if (index == -1)
  {
    text Text2;
    char *url2 = strdup(articleURL);
    char *title2 = strdup(articleTitle);
    // possible change 
     makeText(&Text2, url2, title2, 1);
    VectorAppend(wordsM->texts, &Text2);
  }
  if (index != -1)
  {
    text *vec = (text *)VectorNth(wordsM->texts, index);
    vec->wordsN += 1;
  }
                        ...
                        const char *unused, const char *articleURL, hashset *stopWords, hashset *visitedText, hashset *hashedWords)
{
  ...
  // see if seen
  bool tempBool = compareTexts(visitedText, articleTitle, articleURL);
  if (tempBool == false)
    return;

  char *url = strdup(articleURL);
  HashSetEnter(visitedText, &url);
  char *title = strdup(articleTitle);
  HashSetEnter(visitedText, &title);
  ...
  while (STNextToken(st, word, sizeof(word)))
  {
    ...
    else
    {
      ...
      {
        if (HashSetLookup(stopWords, &curWord) == NULL)
        {
          numWords++;
          if (HashSetLookup(hashedWords, &curWord) == NULL)
          {
            add(hashedWords, curWord, articleTitle, articleURL, stopWords, visitedText);
          }
          else
          {
            wordHash* temp = HashSetLookup(hashedWords, &curWord);
            // renew form here
            renew(articleTitle, articleURL, temp, visitedText, stopWords);
          }
...
static void QueryIndices(hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  ...
  while (true)
  {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, stopWords, hashedWords, visitedText);
  }
}
...
static void ProcessResponse(const char *word, hashset *stopWords, hashset *hashedWords, hashset *visitedText)
{
  if (WordIsWellFormed(word)){
    ...
    {
      if (HashSetLookup(hashedWords, &word) == NULL)
      {
        printf("None of today's news articles contain the word \"%s\".\n", word);
      }
      else
      {
        wordHash *temp = HashSetLookup(hashedWords, &word);

        int num = VectorLength(temp->texts);
        if (num < 10)
        {
          // do nothing
        }
